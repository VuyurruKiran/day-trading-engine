...
**Last Project Rule Update:** 2026-09-03 — Evidence-integrity contract added: verified-history readiness, a separate completeness gate, mandatory catalyst status for PRIMARY, fundamentals as risk context, Reddit optional-only, all-30 pre-finalist enrichment/outcomes, frozen-vs-enhanced validation before weight changes, and explicit finalist evidence visibility.
...
- Catalyst tests must distinguish completed `NO_MATERIAL_CATALYST_FOUND` from source/check unavailable and prove unavailable catalyst evidence blocks PRIMARY.
...
- UI/research-output tests must expose completeness, freshness, provider/feed, history length/status, catalyst state, and rejection/PRIMARY-ineligible reasons for finalists.
...
- Catalyst evidence must distinguish `MATERIAL_CATALYST_FOUND`, `NO_MATERIAL_CATALYST_FOUND`, and unavailable/incomplete. A completed no-catalyst result is valid; unavailable/not-run catalyst/news evidence is not neutral and blocks PRIMARY.
...
- Context scoring keeps normalized technical 50%, market/sector 20%, news 20%, Reddit 5%, fundamentals 5% while this evidence-integrity change is evaluated. The older blanket rule that missing news is neutral and its weight moves to technical is superseded for unavailable/not-run catalyst/news evidence. Reddit remains optional and may be neutral when absent.
