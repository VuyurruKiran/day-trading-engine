...
- Catalyst checking is required for PRIMARY eligibility and must cover current news, earnings, SEC filings, halts/resumptions, offerings/dilution, and major configured company events. `NO_MATERIAL_CATALYST_FOUND` is a valid completed result; catalyst/news unavailable, stale, failed, or not-run is not neutral and makes the candidate `PRIMARY_INELIGIBLE`.
- Fundamentals are intraday risk context: market cap, float/shares context, dilution/offering risk, earnings date/proximity, and basic point-in-time financial-health flags. Deep valuation analysis is not required for Software V1.
- Reddit remains optional sentiment/attention evidence only. Missing Reddit cannot block operation; positive Reddit cannot independently qualify a stock, rescue an incomplete candidate, override a hard gate, or create PRIMARY eligibility.
...
**Last Project Rule Update:** 2026-09-03 — Evidence-integrity ranking semantics clarified: incomplete catalyst/news evidence gets no enhanced score/rank and cannot become PRIMARY; the legacy neutral/weight-reassignment behavior is allowed only inside the frozen comparator used for controlled comparison.
...
- Catalyst tests must distinguish completed `NO_MATERIAL_CATALYST_FOUND` from source/check unavailable and prove any applicable catalyst category/source that is unavailable, stale, failed, or not-run yields an incomplete catalyst state, no enhanced score/rank, and blocks PRIMARY.
- Reddit tests must prove missing Reddit is optional and positive Reddit cannot independently qualify/rescue a candidate.
...
- Frozen-vs-enhanced tests must prove the legacy neutralization/weight-reassignment behavior, if exercised, is confined to the frozen comparator and never leaks into the enhanced or actionable path.
...
- Catalyst evidence must distinguish `MATERIAL_CATALYST_FOUND`, `NO_MATERIAL_CATALYST_FOUND`, and unavailable/incomplete. A completed no-catalyst result is valid; unavailable, stale, failed, or not-run catalyst/news evidence is not neutral and blocks PRIMARY.
- Fundamentals are limited to risk context for V1: market cap, float/shares context, dilution/offering risk, earnings proximity/date, and basic point-in-time financial-health flags. Deep valuation is not required.
- Context scoring keeps normalized technical 50%, market/sector 20%, news 20%, Reddit 5%, fundamentals 5% while this evidence-integrity change is evaluated. In the enhanced method, unavailable/stale/failed/not-run catalyst/news evidence makes the row ranking-incomplete: preserve the research row with `enhanced_score = null` and `enhanced_rank = null`, and exclude it from finalist/PRIMARY selection. The legacy neutralization/weight-to-technical behavior may be reproduced only inside the frozen comparator for historical comparison; its score/rank must never be copied into the enhanced or actionable path. Reddit remains optional and may be neutral when absent.
